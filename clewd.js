...
/******************************** */
                    const newtokenizer = /claude-(2\.1-|[3-9])/.test(model), messagesAPI = oaiAPI || newtokenizer && !/<\|completeAPI\|>/.test(prompt) || /<\|messagesAPI\|>/.test(prompt), messagesLog = /<\|messagesLog\|>/.test(prompt), fusion = apiKey && messagesAPI && /<\|Fusion Mode\|>/.test(prompt), prefix = '\n\u001B\n';
                    ...
                    Config.Settings.FullColon && (prompt = newtokenizer ?
                        prompt.replace(fusion ? /\n\n(?!Assistant:\s*$)(?=Human:|Assistant:)/gs : apiKey ? /(?<!\n\nHuman:.*)\n\n(?=Assistant:)|\n\n(?=Human:)(?!.*\n\nAssistant:)/gs : /\n\n(?=Human:|Assistant:)/g, prefix) : 
                        prompt.replace(fusion ? /(?<=\n\nAssistant):(?!\s*$)|(?<=\n\nHuman):/gs : apiKey ? /(?<!\n\nHuman:.*)(?<=\n\nAssistant):|(?<=\n\nHuman):(?!.*\n\nAssistant:)/gs : /(?<=\n\n(Human|Assistant)):/g, '：'));
                            ...
                            if (messagesAPI) {
                                ...
                                messages = rounds.slice(1).flatMap(round => {
                                    ...
                                    if (Config.Settings.FullColon && acc.length > 0 && (acc[acc.length - 1].role === current.role || !acc[acc.length - 1].content)) {
                                        acc[acc.length - 1].content += (current.role === 'user' ? 'Human' : 'Assistant').replace(/.*/, newtokenizer ? prefix + '$&: ' : '\n\n$&： ') + current.content;
